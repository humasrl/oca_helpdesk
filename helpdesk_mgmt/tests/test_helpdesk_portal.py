...
class TestHelpdeskPortal(TestHelpdeskPortalBase):
    ...
    def test_close_ticket(self):
        ...
        self.authenticate("portal", "portal")
        resp = self.url_open(f"/my/ticket/{self.portal_ticket.id}")
        # Number of close buttons depends on team/stage configuration
    ...
    def test_ticket_list_unauthenticated(self):
        """Attempt to list tickets without auth, ensure we get redirected."""
        resp = self.url_open("/my/tickets", allow_redirects=False)
        self.assertEqual(resp.status_code, 303)
        self.assertTrue(resp.is_redirect)
        # In Odoo 19, redirect may be to language-prefixed URL or login
        location = resp.headers["Location"]
        self.assertTrue(
            "/web/login" in location or "/my/tickets" in location,
            f"Expected redirect to login or tickets, got: {location}",
        )

    def test_ticket_list_authorized(self):
        """Attempt to list tickets without helpdesk permissions."""
        self.authenticate("test-basic-user", "test-basic-user")
        resp = self.url_open("/my/tickets", allow_redirects=True)
        # In Odoo 19, there may be language redirect first (303->200)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_tickets_2_users(self):
        ...
        self.assertEqual(resp.status_code, 303)
        self.assertTrue(resp.is_redirect)
        # In Odoo 19, redirect may include language prefix
        self.assertIn("/my", resp.headers["Location"])
